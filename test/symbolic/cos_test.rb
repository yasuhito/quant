...
module Symbolic
  class CosTest < ActiveSupport::TestCase
    ...
    test 'Cos(π/6) → √3/2' do
      assert_equal Sqrt(3)/2, Cos(Product(1/6, PI)).simplify
    end
    ...
    test 'Cos(5π/6) → -√3/2' do
      assert_equal Product(-1, Sqrt(3)/2), Cos(Product(5/6, PI)).simplify
    end
    ...
    test 'Cos(7π/6) → -√3/2' do
      assert_equal Product(-1, Sqrt(3)/2), Cos(Product(7/6, PI)).simplify
    end
    ...
    test 'Cos(4π/3) → -1/2' do
      assert_equal (-1/2), Cos(Product(4/3, PI)).simplify
    end
    ...
    test 'Cos(11π/6) → √3/2' do
      assert_equal Sqrt(3)/2, Cos(Product(11/6, PI)).simplify
    end
    ...
    test 'Cos(-π/6) → √3/2' do
      assert_equal Sqrt(3)/2, Cos(Product(-1, Product(1/6, PI))).simplify
    end
    ...
    test 'Cos(-5π/6) → -√3/2' do
      assert_equal Product(-1, Sqrt(3)/2), Cos(Product(-1, Product(5/6, PI))).simplify
    end
