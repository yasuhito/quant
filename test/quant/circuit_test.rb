...
module Quant
  class CircuitTest < ActiveSupport::TestCase
    ...
    test 'State flip: |0> to |1> and vice versa' do
      ...
      assert_equal 'β|0> + α|1>', circuit.x(0).to_s
    end
    ...
    test 'Basis change: |0> to |+> and |1> to |−> (and vice versa)' do
      ...
      assert_equal '(2^(-1/2)*α + 2^(-1/2)*β)|0> + (2^(-1/2)*α - 2^(-1/2)*β)|1>', circuit.h(0).to_s
    end
    ...
    test 'Sign flip: |+> to |−> and vice versa' do
      ...
      assert_equal 'α|0> - β|1>', circuit.z(0).to_s
    end
    ...
    test 'Amplitude change: |0> to cos(α)|0⟩ + sin(α)|1⟩' do
      ...
      assert_equal '(cos(α)*β - cos(α)*γ)|0> + (cos(α)*β + cos(α)*γ)|1>', circuit.ry(0, theta: 2 * :α).to_s
    end
    ...
    test 'Phase flip' do
      ...
      assert_equal 'α|0> + β*i|1>', circuit.s(0).to_s
    end
