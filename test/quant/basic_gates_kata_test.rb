...
module Quant
  class BasicGatesKataTest
    class SingleQubitGatesTest < ActiveSupport::TestCase
      using Symbo
      include Symbo
    ...
    class MultiQubitGatesTest < ActiveSupport::TestCase
      using Symbo
      include Symbo
      ...
      test 'Toffoli gate: α|110⟩ → α|111⟩ and β|111⟩ → β|110⟩' do
        circuit = Circuit.new(:α * Qubit['000'] + :β * Qubit['001'] + :γ * Qubit['010'] + :δ * Qubit['011'] + :ϵ * Qubit['100'] + :ζ * Qubit['101'] + :η * Qubit['110'] + :θ * Qubit['111'])
        state = circuit.ccnot([0, 1] => 2).state

        assert_equal :α * Qubit['000'] + :β * Qubit['001'] + :γ * Qubit['010'] + :δ * Qubit['011'] + :ϵ * Qubit['100'] + :ζ * Qubit['101'] + :θ * Qubit['110'] + :η * Qubit['111'], state
        assert_equal 'α|000⟩ + β|001⟩ + γ|010⟩ + δ|011⟩ + ϵ|100⟩ + ζ|101⟩ + θ|110⟩ + η|111⟩', state.to_s
      end

      test 'Fredkin gate: α|101⟩ → α|110⟩ and β|110⟩ → β|101⟩' do
        circuit = Circuit.new(:α * Qubit['000'] + :β * Qubit['001'] + :γ * Qubit['010'] + :δ * Qubit['011'] + :ϵ * Qubit['100'] + :ζ * Qubit['101'] + :η * Qubit['110'] + :θ * Qubit['111'])
        state = circuit.cnot(1 => 2).ccnot([0, 2] => 1).cnot(1 => 2).state

        assert_equal :α * Qubit['000'] + :β * Qubit['001'] + :γ * Qubit['010'] + :δ * Qubit['011'] + :ϵ * Qubit['100'] + :ζ * Qubit['110'] + :η * Qubit['101'] + :θ * Qubit['111'], state
        assert_equal 'α|000⟩ + β|001⟩ + γ|010⟩ + δ|011⟩ + ϵ|100⟩ + η|101⟩ + ζ|110⟩ + θ|111⟩', state.to_s
      end
    end
  end
end
